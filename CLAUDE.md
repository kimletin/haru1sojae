@AGENTS.md

# 하루1소재 프로젝트 규칙

## 절대 규칙

- **`.env` 파일은 절대 커밋하지 않는다.** `.env`, `.env.local`, `.env.*.local` 등 환경변수 파일은 어떤 경우에도 git에 추가하지 않는다.
- **git 커밋은 사용자가 명시적으로 요청할 때만 한다.** 임의로 커밋하지 않는다.

## 코드 수정 전 확인 필수

코드를 수정하기 전에 반드시 사용자에게:
1. **어떤 작업을 할 것인지** (변경 대상 파일, 변경 내용 요약)
2. **어떤 결과가 나오는지** (예상 동작 변화)

를 먼저 설명하고 확인을 받은 후 진행한다.

단, 사용자가 명확하게 "해줘"라고 지시한 단순 작업(예: 특정 텍스트 변경, 색상 변경 등)은 바로 진행해도 된다.

## 파일 편집 안전 규칙

- `page.tsx`·`ExpContentsTab.tsx`처럼 큰 파일도 **Edit 툴로 수정한다.** (Python `data.replace`는 Bash 경유라 토큰을 많이 쓰면 분류기가 막혀 실행이 거부되는 경우가 있음 → Edit 툴이 더 안정적)
- Edit 시 `old_string`은 **충분히 고유한 범위**로 잡는다. 큰 블록을 제거/이동할 때는 시작·끝 경계가 명확한 단위(예: `{selected === 'X' && (...)}` 블록 전체)로 한 번에 처리한다.
- 수정 후 항상 `npx tsc --noEmit`으로 타입 에러를 확인한다.

## 분류기(안전성 검사) 막힘 규칙

- Bash/터미널 명령이 **"분류기(classifier) 일시 불가"**로 거부되면, 이는 보통 **토큰 소진**이 원인이다(잠시 후 회복됨).
- 이 경우 **다른 우회 방법을 찾거나 곧바로 재시도하지 않는다.** 사용자에게 "분류기가 막혀 실행이 거부됐다"고 알리고, 사용자가 다시 시도하라고 할 때까지 기다린다.
- 특히 같은 명령을 연속으로 반복 시도해 토큰을 더 소모하지 않는다.

## 검증 / 서버 실행 규칙

- **dev 서버나 preview 도구를 직접 띄우지 않는다.** 사용자가 자기 dev 서버를 띄워 두고 작업한다.
- **node 프로세스를 전체 종료하지 않는다.** 특히 `taskkill /IM node.exe` 류는 사용자 dev 서버까지 죽이므로 절대 금지.
- 변경 검증은 `npx tsc --noEmit`(또는 `npm run build`)로 끝낸다. 화면 확인이 필요하면 사용자에게 맡기고 말로 설명한다.

## 기술 스택

- Next.js App Router, TypeScript, Tailwind CSS v4
- 라우팅: `app/[[...slug]]/page.tsx` 단일 catch-all 라우트
